...
export const createAzureLanguageSummarier = (): Summarizer => {
  const env = cleanEnv(process.env, {
    AZURE_LANGUAGE_ENDPOINT: str({
      desc: "The endpoint for a Language model of the Azure Cognitive Service",
    }),
    AZURE_LANGUAGE_KEY: str({
      desc: "The key for a Language model of the Azure Cognitive Service",
    }),
  });
  ...
  return {
    ...
    summarize: async (text: string, languageCode: string): Promise<SummaryResult[]> => {
      const mappedLanguageCode =
        azureLanguageCodeMap[languageCode as keyof typeof azureLanguageCodeMap];
      ...
      for await (const actionResult of results) {
        if (actionResult.kind !== "AbstractiveSummarization") {
          throw new Error(
            `Expected abstractive summarization results but got: ${actionResult.kind}`,
          );
        }
